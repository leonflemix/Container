...
const renderContainers = () => {
    ...
    if (containers.length === 0) {
        ...
        containers.forEach(c => {
            ...
            row.innerHTML = `<td class="px-6 py-4 font-semibold text-gray-900">${c.serial}</td><td class="px-6 py-4">${c.type || 'N/A'}</td><td class="px-6 py-4">${getLocationIcon(c.location)}</td><td class="px-6 py-4">${getStatusBadge(c.status)}</td><td class="px-6 py-4">${c.driver || 'N/A'}</td><td class="px-6 py-4 text-gray-500">${formatTimeAgo(c.lastUpdated)}</td><td class="px-6 py-4 text-center"><button data-id="${c.id}" class="edit-btn font-medium text-blue-600 hover:underline">Update</button></td>`;
...
const handleDriverFormSubmit = async (e) => {
    ...
    try { await addDoc(collection(db, `/artifacts/${window.appId}/public/data/drivers`), driverData); e.target.reset(); } catch (error) { console.error(`Error adding driver:`, error); }
};
...
const handleChassisFormSubmit = async (e) => {
    ...
    try { await addDoc(collection(db, `/artifacts/${window.appId}/public/data/chassis`), chassisData); e.target.reset(); } catch (error) { console.error('Error adding chassis:', error); }
};
...
const handleStatusFormSubmit = async (e) => {
    ...
    try { await addDoc(collection(db, `/artifacts/${window.appId}/public/data/statuses`), statusData); e.target.reset(); } catch (error) { console.error('Error adding status:', error); }
};
...
const handleEditFormSubmit = async (e) => {
    ...
    for (let [key, value] of formData.entries()) {
        if (key !== 'id' && key !== 'collection') {
            updatedData[key] = (key === 'weight' || key === 'qty') ? Number(value) : value;
        }
    }
...
const addCollectionItem = async (collectionName, value) => {
    if (!value) return;
    try { await addDoc(collection(db, `/artifacts/${window.appId}/public/data/${collectionName}`), { name: value }); } catch (error) { console.error(`Error adding to ${collectionName}:`, error); }
};
...
const setupEventListeners = () => {
    ...
    document.getElementById('settings-page').addEventListener('click', (e) => {
     ...
     document.getElementById('logistics-page').addEventListener('click', (e) => {
        const target = e.target.closest('button');
        if (!target) return;
        if (target.classList.contains('edit-item-btn')) { openEditModal(target.dataset.collection, target.dataset.id); }
        if (target.classList.contains('delete-item-btn')) { deleteCollectionItem(target.dataset.collection, target.dataset.id); }
